            ...
            for ekey, edig in self.db.getOoeItemIter(key=key):
                try:
                    ...
                    if not (esr := self.db.esrs.get(keys=dgkey)):  # get event source, otherwise error
                        ...
                        raise ValidationError(f"Missing escrowed event source at dig = {edig}.")
                    ...
                    if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutOOE):
                        ...
                        logger.info(f"Kevery unescrow error: Stale event escrow at dig = {edig}")
                        raise ValidationError(f"Stale event escrow at dig = {edig}.")
                    ...
                    if eraw is None:
                        ...
                        logger.info(f"Kevery unescrow error: Missing event at dig = {edig}")
                        raise ValidationError(f"Missing escrowed evt at dig = {edig}.")
                    ...
                    if not sigs:  # otherwise its a list of sigs
                        ...
                        logger.info(f"Kevery unescrow error: Missing event sigs at dig = {edig}")
                        raise ValidationError(f"Missing escrowed evt sigs at dig = {edig}.")
    ...
    def processEscrowPartialSigs(self):
        ...
        for ekey, edig in self.db.getPseItemIter(key=b''):
            ...
            try:
                ...
                if not (esr := self.db.esrs.get(keys=dgkey)):  # get event source, otherwise error
                    # no local sourde so raise ValidationError which unescrows below
                    raise ValidationError("Missing escrowed event source at dig = {edig}.")

                # check date if expired then remove escrow.
                dtb = self.db.getDts(dgkey)
                if dtb is None:  # othewise is a datetime as bytes
                    # no date time so raise ValidationError which unescrows below
                    logger.info(f"Kevery unescrow error: Missing event datetime at dig = {edig}")
                    raise ValidationError(f"Missing escrowed event datetime at dig = {edig}.")
            ...
            for (pre, said), edig in self.db.qnfs.getItemIter(keys=key):
                try:
                    ...
                    if (dtnow - dte) > datetime.timedelta(seconds=self.TimeoutQNF):
                        # escrow stale so raise ValidationError which unescrows below
                        logger.info(f"Kevery unescrow error: Stale qry event escrow at dig = {edig}")

                        raise ValidationError(f"Stale qry event escrow at dig = {edig}.")
