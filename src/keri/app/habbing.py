    ...
    def loadHabs(self):
        ...
        for prefix, habord in self.db.habs.getItemIter():
            ...
            if habord.mid and not habord.sid:
                ...
                hab = SignifyGroupHab(smids=habord.smids, ks=self.ks, db=self.db, cf=self.cf,
                                      mgr=self.mgr, rtr=self.rtr, rvy=self.rvy, kvy=self.kvy,
                                      psr=self.psr, name=habord.name, pre=pre)
                groups.append(habord)
            else:
