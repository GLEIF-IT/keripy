...
OUT=$(kli vc list --name multisig1 --alias multisig --issued)
echo "$OUT"

if echo "$OUT" | grep -q 'Status: Revoked'; then
  echo "Assertion passed: Credential is revoked."
else
  echo "Assertion failed: Credential is not revoked."
  exit 1
fi
