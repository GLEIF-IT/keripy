...
def test_tocking_specific_config_precedes_coarse_config():
    ...
    # cascaded from pollerEvent coarse grain config
    assert tocks["pollerConnect"] == 0.4
    assert tocks["pollerRequest"] == 0.4
    assert tocks["pollerMessage"] == 0.4
    # specific config overrides coarse config
    assert tocks["pollerIdle"] == 0.125
...
def test_tocking_environment_precedes_config_and_coarse_environment():
    tocks = tocking.resolveTocks(
        ...
        environ={
            ...
            "KERI_POLLER_REQUEST_TOCK": "0.25",
            "KERI_VDR_ESCROW_TOCK": "0.4",
            "KERI_REGISTRAR_ESCROW_TOCK": "0.125",
        },
    )

    assert tocks["pollerConnect"] == 0.5
    assert tocks["pollerRequest"] == 0.25
    assert tocks["pollerMessage"] == 0.5
    assert tocks["pollerIdle"] == 0.5
...
def test_tocking_rejects_invalid_coarse_environment_when_specifics_shadow_it():
    with pytest.raises(kering.ConfigurationError,
                       match="KERI_POLLER_EVENT_TOCK"):
        tocking.resolveTocks(
            environ={
                "KERI_POLLER_EVENT_TOCK": "not-a-number",
                "KERI_POLLER_CONNECT_TOCK": "0.1",
                "KERI_POLLER_REQUEST_TOCK": "0.2",
                "KERI_POLLER_MESSAGE_TOCK": "0.3",
                "KERI_POLLER_IDLE_TOCK": "0.4",
            }
        )
